...
def test(model,
    ...
    metric_group = MetricGroup(Acc=args.test_config["METRIC"]["Acc"],
                               mIoUD=args.test_config["METRIC"]["mIoUD"],
                               mIoUIC=args.test_config["METRIC"]["mIoUIC"],
                               mIoUK=args.test_config["METRIC"]["mIoUK"],
                               ...
                               reduce_panoptic_zero_label= \
                                   args.data_config["reduce_panoptic_zero_label"],
                               threshold=args.test_config["METRIC"]["threshold"],
                               q=args.test_config["METRIC"]["q"],
                               q_bar=args.test_config["METRIC"]["q_bar"])
    ...
    for iter, (image,
               ...
               panoptic,
               image_file,
               panoptic_file) in enumerate(data_loader):
        start = time.time()
...
def test_medical(model,
    ...
    if BDice:
        levels = [i / 10 for i in range(1, 10, 1)]
        BDice_value = 0
    else:
        levels = [None]
    ...
    for level in levels:
        if BDice:
            logging.info(f"Test level {level}")
        ...
        for case in range(args.data_config["num_cases"] + 1):
            metric_groups[case] = MetricGroup(Acc=args.test_config["METRIC"]["Acc"],
                                              mIoUD=args.test_config["METRIC"]["mIoUD"],
                                              mIoUIC=args.test_config["METRIC"]["mIoUIC"],
                                              mIoUK=args.test_config["METRIC"]["mIoUK"],
                                              ECED=args.test_config["METRIC"]["ECED"],
        ...
        for iter, (image,
            ...
            if "qubiq" in args.data_config["dataset"]:
                ...
                if BDice:
                    label = torch.where(label >= level, 1, 0)
                else:
            ...
            if not BDice and args.test_config["ITER"]["save_pred"]:
                save_pred(prob,
                          label,
                          image_file,
                          dataset.color_map,
        ...
        with open(os.path.join(args.output_dir, f"metric_groups{level}.pkl"), "wb") as pkl:
            pickle.dump(metric_groups, pkl)

    if BDice:
        BDice_value /= len(levels)
        logging.info(f"BDice: {BDice:.2f}")
