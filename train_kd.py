...
def train_kd(model, teacher, data_loader, optimizer, scheduler, device, epoch, args):
    ...
    for iter, (image, label, _) in enumerate(data_loader):
        ...
        prob_teacher, aux_prob_teacher = get_soft_label(teacher, image, label, args)

        optimizer.zero_grad()
        loss_ce, loss_jdt = model.forward_loss_kd(image, label, prob_teacher, aux_prob_teacher, keep_mask)
...
def get_soft_label(teacher, image, label, args):
    with torch.no_grad():
        logits_teacher, aux_logits_teacher = teacher(image)
        prob_teacher = (logits_teacher / args.label_config["T"]).log_softmax(dim=1).exp()
        aux_prob_teacher = (aux_logits_teacher / args.label_config["T"]).log_softmax(dim=1).exp()
        ...
        if args.label_config["kde"]:
            ...
            label_boundary = label_boundary.unsqueeze(1).expand(batch_size, args.data_config["num_classes"], crop_h, crop_w)
            prob_teacher, aux_prob_teacher = kde(prob_teacher, aux_prob_teacher, label, label_boundary, args)
