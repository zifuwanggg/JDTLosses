# modified from https://github.com/facebookresearch/MaskFormer/blob/main/datasets/prepare_ade20k_pan_seg.py
...
if __name__ == "__main__":
    ...
    dataset_dir = os.path.join(sys.argv[1], "ADEChallengeData2016")
    for name, dirname in [("train", "training"), ("val", "validation")]:
        image_dir = os.path.join(dataset_dir, f"images/{dirname}/")
        semantic_dir = os.path.join(dataset_dir, f"annotations/{dirname}/")
        instance_dir = os.path.join(
            dataset_dir, f"annotations_instance/{dirname}/"
        )

        # folder to store panoptic PNGs
        out_folder = os.path.join(dataset_dir, f"ade20k_panoptic_{name}/")
        # json with segmentations information
        out_file = os.path.join(dataset_dir, f"ade20k_panoptic_{name}.json")
        ...
        # json config
        config_file = os.path.join(dataset_dir, "ade20k_instance_imgCatIds.json")
        with open(config_file) as f:
            config = json.load(f)

        # load catid mapping
        mapping_file = os.path.join(dataset_dir, "ade20k_instance_catid_mapping.txt")
