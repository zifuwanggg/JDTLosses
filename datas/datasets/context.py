...
class Context(BaseDataset):
    ...
    def get_image_list(self):
        ...
        assert len(train_image_list) == 4996, \
            f"`len(train_image_list)`: {len(train_image_list)} does not equal to 4996"

        assert len(val_image_list) == 5104, \
            f"`len(val_image_list)`: {len(val_image_list)} does not equal to 5104"
